...
static struct i2c_driver l3gd20h_gyr_driver = {
	...
	.remove = __devexit_p(l3gd20h_gyr_remove),
	.id_table = l3gd20h_gyr_id,

};

static int __init l3gd20h_gyr_init(void)
{

	pr_info("%s: gyroscope sysfs driver init\n", L3GD20H_GYR_DEV_NAME);

	return i2c_add_driver(&l3gd20h_gyr_driver);
}

static void __exit l3gd20h_gyr_exit(void)
{

	pr_info("%s exit\n", L3GD20H_GYR_DEV_NAME);

	i2c_del_driver(&l3gd20h_gyr_driver);
	return;
}

module_init(l3gd20h_gyr_init);
module_exit(l3gd20h_gyr_exit);
